...
void iiwa_ik::chatterCallback_shoulder(const geometry_msgs::PoseStamped &msg)
{
	Shoulder_pos(0) = msg.pose.position.x;
	Shoulder_pos(1) = msg.pose.position.y;
	Shoulder_pos(2) = msg.pose.position.z;
	// cout << "Shoulder" << Shoulder_pos << endl;
}

void iiwa_ik::chatterCallback_hand(const geometry_msgs::PoseStamped &msg)
{
	Hand_pos(0) = msg.pose.position.x;
	Hand_pos(1) = msg.pose.position.y;
	Hand_pos(2) = msg.pose.position.z;
}
...
void iiwa_ik::Topic_initialization()
{
	...
	sub_hand = n->subscribe("/Hand_robot/pose", 3, &iiwa_ik::chatterCallback_hand, this);
	sub_shoulder = n->subscribe("/Shoulder_robot/pose", 3, &iiwa_ik::chatterCallback_shoulder, this);
	sub_base = n->subscribe("/Robot_base/pose", 3, &iiwa_ik::chatterCallback_base, this);

	pub_command = n->advertise<std_msgs::Int64>("/command", 3);
	pub_grab = n->advertise<std_msgs::Int8>("/grab", 3);
}
...
RobotInterface::Status iiwa_ik::RobotUpdateCore()
{
	...
	ros::spinOnce();
	Eigen::VectorXd jt = JointPos;

	mSKinematicChain->setJoints(jt.data());
	mSKinematicChain->getEndPos(EndPos);
	...
	// 			svm_activate = false;
	// 		}
	// 		if (gamma_dist < 0.05 | svm_activate_neg) //Lin 0.03+0.095+0.05
	// 		{
	// 			SVM_out = SVM.calculateGammaDerivative(EndPos_conv) / 100;
	// 			Desired_EndPos_tmp = EndPos_conv + SVM_out;
	// 			svm_activate_neg = true;
	// 			lin_grad_extra = SVM_out * (0.07 - gamma_dist) * 60;
	// 			if (gamma_dist > 0.07)
	// 			{
	...
	// 		///////////////// Test, since SVM and lin was in different spaces before. Shoulder and hand need to get new frames if this is
	// 		///////////////// Uncommented
	// 		if (gamma_dist > 0.05 & gamma_dist < 0.10)
	// 		{	
	// 			grab_msg.data=1;
	...
	// 				lin_grad_vec = Hand_pos - EndPos_conv;
	// 			}
	// 			else
	// 			{
	// 				tmp_vec << 0, 0, 0.00; //0.05 300
	...
	// 				lin_grad_vec = Shoulder_pos - EndPos_conv;
	// 			}
	// 			if (gamma_dist > 0.075)
	// 			{
	// 				lin_grad_extra = -SVM_out * (gamma_dist - 0.075) * 150;
	// 				// lin_grad_extra = -SVM_out * (gamma_dist - 0.075) * (gamma_dist - 0.075) * 10000;
	// 			}
	// 			else if (gamma_dist <= 0.075)
	// 			{
	// 				lin_grad_extra = SVM_out * (0.075 - gamma_dist) * 150;
	// 				// lin_grad_extra = SVM_out * (0.075 - gamma_dist) * (0.075 - gamma_dist) * 10000;
	// 			}
	// 			else
	// 			{
	// 				lin_grad_extra << 0, 0, 0;
	// 			}
	// 			lin_grad_vec = lin_grad_vec - lin_grad_vec.dot(-gamma_vec) * lin_grad_vec;
	// 			if (lin_grad_vec.norm() > 0.02)
	// 			{

	// 				lin_grad_vec = (0.02 / lin_grad_vec.norm()) * lin_grad_vec;
	// 			}

	// 			///// Predictive motion planner
	// 			gamma_dist_tmp = SVM.calculateGamma(EndPos_conv + lin_grad_vec);
	// 			lin_grad_extra << 0,0,0;
	// 			while (abs(gamma_dist_tmp-0.075)>0.001)
	// 			{
	// 				lin_grad_extra = lin_grad_extra+SVM_out*((gamma_dist_tmp<0.075)-(gamma_dist_tmp>0.075))/10;
	// 				gamma_dist_tmp = SVM.calculateGamma(EndPos_conv + lin_grad_vec + lin_grad_extra);
	// 				cout<<gamma_dist_tmp<<endl;
	// 			}
	// 			//////

	// 			cout << "lin_grad" << lin_grad_vec << endl;
	// 			cout << "lin_grad extra" << lin_grad_extra << endl;
	// 			Desired_EndPos_tmp = EndPos_conv + lin_grad_vec + lin_grad_extra;
	// 			cout << "target " << target << endl;

	// 			// if (abs(EndPos_conv(1) - target(1)) < 0.1) //Hand +0.15
	// 			target_tmp=EndPos_conv - target;
	// 			if (target_tmp.norm() < 0.15) //Hand +0.15
	// 			{
	...
	// 		if (gamma_dist > 0.10 | svm_activate == true | svm_activate_neg == true) //Lin 0.03+0.095+0.05
	// 		{
	...
	// 		}
	// 		// if (gamma_dist < 0.12 && gamma_dist > 0.05 && svm_activate == false && svm_activate_neg == false)
	// 		if (gamma_dist < -0.10 && gamma_dist > 0.05) //Lin 0.03+0.095+0.05 0.01+0.095+0.05
	// 		{
	...
	// 				cout << "lin_grad" << lin_grad_vec << endl;
	// 				cout << "lin_grad extra" << lin_grad_extra << endl;
	// 				Desired_EndPos = EndPos + lin_grad_vec + lin_grad_extra;
	...
	// 	Desired_JointPos = JointPos; //0.85 * Desired_JointPos + 0.15 * (JointPos + Desired_JointVel * dt * 0.5);
	// 	break;
	// case PLANNER_JOINT:
